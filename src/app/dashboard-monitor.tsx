...
function MetricTile({
  ...
  const tones = {
    teal: "border-[#c9b187]/55 bg-[#f3ead4]/80 text-[#2e2418]",
    blue: "border-[#5a6488]/45 bg-[#eef0f6]/82 text-[#2a3354]",
    amber: "border-[#d6a850]/55 bg-[#f7ecd0]/82 text-[#5a4418]",
    red: "border-[#c2604f]/55 bg-[#f6e2dc]/82 text-[#7a2820]",
    green: "border-[#94a258]/50 bg-[#eef1df]/80 text-[#46522a]",
  };

  return (
    <section className={`metric-tile flex h-full min-h-0 flex-col rounded-lg border p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] ${tones[tone]}`}>
      <div className="text-sm font-bold text-[#6a5641]">{title}</div>
      <div className="metric-value mt-auto pt-4 font-mono text-4xl font-black leading-none xl:text-5xl">{value}</div>
      <div className="mt-4 text-xs font-medium leading-5 text-[#8a7355]">{caption}</div>
    </section>
  );
}
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="tech-stat rounded-md border border-[#c9b187]/40 bg-[#fbf6ea]/82 px-3 py-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)]">
      <div className="text-xs font-semibold text-[#6a5641]">{label}</div>
      <div className="mt-1.5 font-mono text-lg font-black leading-none text-[#2e2418]">{value}</div>
    </div>
  );
}
...
function Panel({
  ...
  return (
    <section className={`monitor-panel flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-[#c9b187]/55 bg-[#fbf6ea]/88 p-4 ${className}`}>
...
function Gauge({ value, label }: { value: number; label: string }) {
  return (
    <div className="grid place-items-center">
      <div
        className="gauge-ring-paper grid aspect-square w-full max-w-56 place-items-center rounded-full p-4 shadow-[0_6px_22px_-12px_rgba(162,58,46,0.4)]"
        style={{ "--score": value } as CSSProperties}
      >
        <div className="grid h-full w-full place-items-center rounded-full border border-[#c9b187]/55 bg-[#fbf6ea] text-center shadow-[inset_0_0_24px_rgba(205,138,50,0.12)]">
          <div>
            <div className="text-sm font-bold text-[#8a7355]">{label}</div>
            <div className="mt-2 font-mono text-4xl font-black text-[#a23a2e]">{formatPercent(value)}</div>
          </div>
        </div>
      </div>
    </div>
...
function CompactAttendanceLine({
  ...
  return (
    <div className="rounded-md bg-[#f4eccf]/60 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-xs font-black text-[#6a5641]">{title}</div>
          <div className="mt-1 font-mono text-3xl font-black leading-none text-[#2e2418]">{formatPercent(rate)}</div>
        </div>
        <div className="shrink-0 text-right text-xs font-semibold leading-5 text-[#8a7355]">
          {formatNumber(total)}건
        </div>
      </div>
      <div className="mt-3 grid grid-cols-4 gap-2">
        {items.map(([label, value]) => (
          <div key={label} className="min-w-0 rounded-md bg-[#e7d9bc]/70 px-2 py-2">
            <div className="truncate text-[11px] font-semibold text-[#6a5641]">{label}</div>
            <div className="mt-1 font-mono text-lg font-black leading-none text-[#2e2418]">{formatNumber(value)}</div>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="overview-attendance-main min-w-0">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="truncate text-xs font-black text-[#5a4418]">{title}</div>
            <div className="mt-1 flex min-w-0 items-center gap-2">
              <span className="attendance-rate-chip shrink-0 rounded-full px-2 py-0.5 text-[11px] font-black">
                {chipLabel}
              </span>
              <span className="truncate font-mono text-xs font-bold text-[#6a5641]">{formatNumber(total)}건</span>
            </div>
          </div>
        </div>
        <div className="overview-attendance-rate mt-2 font-mono text-3xl font-black leading-none text-[#2e2418]">
          {formatPercent(rate)}
        </div>
      </div>
      <div className="overview-attendance-breakdown grid min-w-0 grid-cols-4 gap-2">
        {items.map(([label, value, tone]) => (
          <div key={label} className={`attendance-metric-pill attendance-metric-${tone} min-w-0 rounded-md px-2.5 py-2`}>
            <div className="truncate text-[11px] font-bold text-[#6a5641]">{label}</div>
            <div className="mt-1 font-mono text-base font-black leading-none text-[#2e2418]">{formatNumber(value)}</div>
          </div>
        ))}
      </div>
    </div>
        ...
        return (
          <div key={item.name}>
            <div className="flex items-center justify-between gap-4 text-sm">
              <span className="truncate font-semibold text-[#2e2418]">{item.name}</span>
              <span className="shrink-0 font-mono text-[#a23a2e]">
                {formatNumber(item.value)}
                {showPercent ? ` · ${percent.toFixed(1)}%` : ""}
              </span>
            </div>
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-[#e7d9bc]/70">
              <div
...
function AbsenceObservationList({ summary, limit = 8 }: { summary: DashboardSummary; limit?: number }) {
      ...
      >
        <div className="absence-row-main grid min-w-0 gap-0.5">
          <div className="flex min-w-0 items-baseline gap-2">
            <span className="shrink-0 font-mono text-[12px] font-black leading-none text-[#2e2418]">{item.student.studentNo}</span>
            <span className="truncate text-[11px] font-black text-[#6a5641]">{item.student.department || item.student.program}</span>
          </div>
          <div
            className="absence-course-line truncate text-[10px] font-semibold leading-3 text-[#8a7355]"
        ...
        <div className="absence-counts-inline flex shrink-0 items-center gap-1">
          {counts.map(([label, value, metric]) => (
            ...
            >
              <span className="font-sans text-[9px] font-bold text-[#8a7355]">{label}</span>
  ...
  if (targets.length === 0) {
    return (
      <div className="rounded-md border border-[#c9b187]/45 bg-[#f4eccf]/60 p-4 text-sm font-semibold text-[#8a7355]">
  ...
  return (
    <div className="grid h-full min-h-0 content-between gap-2">
      {summary.recentClasses.slice(0, limit).map((session) => (
        <div
          key={session.id}
          className="flex min-h-0 items-center justify-between gap-3 rounded-md border border-[#c9b187]/40 bg-[#f4eccf]/56 px-3 py-2"
        >
          <div className="min-w-0">
            <div className="truncate text-sm font-bold text-[#2e2418]">{session.courseName}</div>
            <div className="mt-1 text-xs text-[#8a7355]">
              {session.courseCode} · {session.room}
            </div>
          </div>
          <div className="shrink-0 font-mono text-xs text-[#a23a2e]">
      ...
      <div className="attendance-right-column grid h-full min-h-0 gap-3 xl:grid-rows-[minmax(0,0.36fr)_minmax(0,0.64fr)]">
        <Panel title="결석 누적 관찰">
          <div className="grid h-full min-h-0 gap-3">
            <div className="text-sm font-semibold leading-6 text-[#8a7355]">
    ...
    <Panel
      ...
      action={
        <button
          ...
          data-mode={mode}
          onClick={() => onModeChange(mode)}
          className="rounded-md bg-[#f0e3c0]/70 px-3 py-2 text-xs font-bold text-[#5a4418] ring-1 ring-[#c9b187]/55 transition-smooth hover:bg-[#ecdcb0]/85 hover:text-[#2e2418]"
        >
          상세내역
        </button>
      }
...
function EnrollmentStatusToggle({
  ...
  return (
    <div className="status-filter-bar flex min-h-0 items-center justify-between gap-3 rounded-lg border border-[#c9b187]/55 bg-[#fbf6ea]/90 px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)]">
      <div className="min-w-0">
        <div className="text-base font-black text-[#2e2418]">재적현황</div>
        <div className="mt-1 font-mono text-xs font-bold text-[#8a7355]">
          {formatNumber(getEnrollmentFilterTotal(summary, activeFilter))}명
        </div>
      </div>
      <div className="grid shrink-0 grid-cols-3 gap-1 rounded-md bg-[#e7d9bc]/70 p-1" role="group" aria-label="재적현황 보기">
          ...
          return (
            <button
              ...
              className={`min-w-20 rounded px-3 py-2 text-xs font-black transition-smooth ${
                active ? "bg-[#a23a2e] text-[#fff8ec]" : "text-[#6a5641] hover:bg-[#f0e3c0]/70 hover:text-[#2e2418]"
              }`}
            >
...
function StatusDetailRows({ items, limit }: { items: DistributionItem[]; limit: number }) {
  ...
  if (shown.length === 0) {
    return (
      <div className="grid h-full place-items-center rounded-md border border-dashed border-[#c9b187]/55 bg-[#f4eccf]/50 text-sm font-bold text-[#8a7355]">
        대상 없음
      </div>
    );
  }
    ...
    <div className="grid h-full min-h-0 gap-2 overflow-y-auto pr-1">
      {shown.map((item, index) => {
        ...
        return (
          <div
            key={item.name}
            className="status-detail-row grid gap-3 rounded-md border border-[#c9b187]/40 bg-[#f4eccf]/56 px-3 py-2.5 sm:grid-cols-[auto_1fr_auto]"
          >
            <div className="font-mono text-xs font-black text-[#a23a2e]">{String(index + 1).padStart(2, "0")}</div>
            <div className="min-w-0">
              <div className="truncate text-sm font-black text-[#2e2418]">{item.name}</div>
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#e7d9bc]/70">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-[#a23a2e] via-[#d98a3a] to-[#e8c46a]"
                  style={{ width: `${Math.max((item.value / max) * 100, 3)}%` }}
                />
            ...
            <div className="text-right font-mono text-sm font-black text-[#a23a2e]">
              {formatNumber(item.value)}
              <div className="mt-1 text-xs text-[#8a7355]">{percent.toFixed(1)}%</div>
            </div>
          </div>
        );
      })}
  ...
  return (
    <button
      ...
      className={`min-w-14 rounded px-2.5 py-2 text-xs font-black transition-smooth ${
        active
          ? "bg-[#a23a2e] text-[#fff8ec]"
          : "bg-[#f4eccf]/70 text-[#6a5641] ring-1 ring-[#c9b187]/45 hover:bg-[#ecdcb0]/85 hover:text-[#2e2418]"
      } disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:bg-[#f4eccf]/70 disabled:hover:text-[#6a5641]`}
    >
      {label}
    </button>
  );
...
function StatusDimensionControl({
  ...
  return (
    <div className="min-w-0">
      <div className="mb-2 text-[11px] font-black text-[#8a7355]">{label}</div>
...
function StatusFilterDimensionControl({
  ...
  return (
    <div className="min-w-0">
      <div className="mb-2 text-[11px] font-black text-[#8a7355]">필터 기준</div>
...
function StatusFilterValueControl({
  ...
  return (
    <label className="min-w-0">
      <div className="mb-2 text-[11px] font-black text-[#8a7355]">필터 값</div>
      <select
        value={effectiveValue}
        disabled={filterDimension === "all" || filterValues.length === 0}
        onChange={(event) => onFilterValueChange(event.target.value)}
        className="h-9 w-full rounded-md border border-[#c9b187]/50 bg-[#fbf6ea] px-3 text-xs font-black text-[#2e2418] outline-none transition-smooth focus:border-[#a23a2e]/65 disabled:opacity-55"
      >
        {filterDimension === "all" ? (
...
function StatusCombinationControls({
  ...
  return (
    <div className="combination-controls grid min-h-0 gap-3 rounded-lg border border-[#c9b187]/55 bg-[#fbf6ea]/90 px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] md:grid-cols-2 2xl:grid-cols-[1fr_1fr_1.2fr_0.9fr]">
    ...
    <div className="grid h-full min-h-0 gap-2 overflow-y-auto pr-1">
      {shown.map((row, index) => {
        ...
        return (
          <div
            key={row.name}
            className="status-detail-row rounded-md border border-[#c9b187]/40 bg-[#f4eccf]/56 px-3 py-2.5"
          >
            <div className="grid items-start gap-3 sm:grid-cols-[auto_1fr_auto]">
              <div className="font-mono text-xs font-black text-[#a23a2e]">{String(index + 1).padStart(2, "0")}</div>
              <div className="min-w-0">
                <div className="truncate text-sm font-black text-[#2e2418]">{row.name}</div>
                <div className="mt-1 font-mono text-xs font-bold text-[#8a7355]">{rowPercent.toFixed(1)}%</div>
              </div>
              <div className="text-right font-mono text-sm font-black text-[#a23a2e]">{formatNumber(row.value)}명</div>
            </div>
                ...
                return (
                  <div key={`${row.name}-${segment.name}`} className="grid gap-1">
                    <div className="flex items-center justify-between gap-3 text-xs">
                      <span className="truncate font-bold text-[#5a4418]">{segment.name}</span>
                      <span className="shrink-0 font-mono font-black text-[#a23a2e]">
                        {formatNumber(segment.value)}명 · {segmentPercent.toFixed(1)}%
                      </span>
                    </div>
                    <div className="h-1.5 overflow-hidden rounded-full bg-[#e7d9bc]/70">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-[#a23a2e] via-[#d98a3a] to-[#e8c46a]"
  ...
  return (
    ...
    >
      ...
      <div className="monitor-grid status-detail-grid grid h-full min-h-0 gap-3 xl:grid-cols-[0.72fr_1.28fr]">
        <Panel title="분포 순위">
          <div className="grid h-full min-h-0 gap-3">
            <div className="rounded-md border border-[#c9b187]/40 bg-[#f4eccf]/60 px-3 py-2">
              <div className="text-xs font-black text-[#8a7355]">조합분석</div>
              <div className="mt-1 text-lg font-black text-[#2e2418]">{title}</div>
              <div className="mt-2 font-mono text-xs font-bold text-[#a23a2e]">
...
function getStatusBadgeClass(status: string) {
  if (status === "관리 필요") {
    return "border-[#a23a2e]/60 bg-[#f6e2dc]/70 text-[#7a2820] shadow-[0_2px_10px_-4px_rgba(162,58,46,0.4)]";
  }

  if (status === "정성 관리") {
    return "border-[#cd8a32]/55 bg-[#f7ecd0]/72 text-[#5a4418]";
  }

  return "border-[#7b8e44]/50 bg-[#eef1df]/66 text-[#46522a]";
}

function getStatusStampColor(status: string) {
  if (status === "관리 필요") return "text-[#a23a2e]";
  if (status === "정성 관리") return "text-[#9a6a18]";
  return "text-[#5a6a3c]";
}
        ...
        <section
          ...
          className={`summary-certification-card flex h-full min-h-0 flex-col rounded-lg border p-3.5 ${
            indicator.status === "관리 필요"
              ? "border-[#c2604f]/50 bg-[#f6e2dc]/80"
              : "border-[#c9b187]/45 bg-[#fbf6ea]/82"
          }`}
        >
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1 whitespace-nowrap text-[clamp(0.72rem,0.82vw,0.875rem)] font-black text-[#5a4418]">
              {indicator.title}
            </div>
            <span
          ...
          <div className="summary-value-stack mt-3 grid gap-2">
            <div className="summary-value min-w-0 font-mono text-[clamp(1.85rem,2.2vw,2.45rem)] font-black leading-none text-[#2e2418]">
              {indicator.value}
            </div>
            <div className="summary-count-ratio-row flex min-w-0 items-center justify-end rounded-md bg-[#e7d9bc]/70 px-2.5 py-1.5">
              <span className="summary-count-ratio whitespace-nowrap text-right font-mono text-sm font-black leading-none text-[#a23a2e]">
                {indicator.count}
              </span>
            </div>
          </div>

          <div className="mt-3 grid gap-2 text-xs font-semibold leading-5 text-[#8a7355]">
            <div>{indicator.criterion}</div>
            <div className="rounded-md bg-[#e7d9bc]/70 px-2 py-1.5 text-[#5a4418]">{indicator.note}</div>
          </div>
        </section>
      ))}
    </div>
...
function IndicatorCard({
  ...
  const activeTone = {
    blue: "border-[#2d3a66]/55 bg-[#eef0f6]/88",
    amber: "border-[#cd8a32]/55 bg-[#f7ecd0]/88",
    red: "border-[#c2604f]/55 bg-[#f6e2dc]/88",
    green: "border-[#7b8e44]/50 bg-[#eef1df]/86",
  };
  ...
  return (
    <button
      ...
      className={`flex h-full min-h-0 flex-col rounded-lg border p-4 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] transition-smooth ${
        active
          ? activeTone[indicator.tone]
          : "border-[#c9b187]/45 bg-[#fbf6ea]/82 hover:border-[#a23a2e]/45 hover:bg-[#f7eed6]/90"
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="text-sm font-black text-[#2e2418]">{indicator.title}</div>
        <span className={`status-stamp shrink-0 text-[10px] ${getStatusStampColor(indicator.status)}`}>
          {indicator.status}
        </span>
      </div>
      <div className="mt-auto flex items-end justify-between gap-3 pt-3">
        <div className="font-mono text-4xl font-black leading-none text-[#2e2418]">{indicator.value}</div>
        <div className="shrink-0 whitespace-nowrap font-mono text-sm font-black leading-none text-[#a23a2e]">
          {indicator.count}
        </div>
      </div>
      <div className="mt-3 text-xs font-semibold leading-5 text-[#8a7355]">{indicator.criterion}</div>
      <div className="indicator-progress mt-3 h-1.5 overflow-hidden rounded-full bg-[#e7d9bc]/70">
        <div
          className="h-full rounded-full bg-gradient-to-r from-[#a23a2e] via-[#d98a3a] to-[#e8c46a]"
...
function CertificationDetail({ indicator }: { indicator: CertificationIndicator }) {
  return (
    <div className="grid h-full min-h-0 gap-3">
      <div className="rounded-md border border-[#c9b187]/45 bg-[#f4eccf]/60 p-3">
        <div className="text-xs font-bold text-[#8a7355]">선택 지표</div>
        <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
          <div className="text-2xl font-black text-[#2e2418]">{indicator.title}</div>
          <div className="font-mono text-3xl font-black text-[#a23a2e]">{indicator.value}</div>
        </div>
      </div>
      ...
      <div className="grid min-h-0 gap-2 text-sm leading-6 text-[#5a4418]">
        <div className="rounded-md border border-[#c9b187]/40 bg-[#f4eccf]/56 px-3 py-2">
          <span className="font-black text-[#2e2418]">산식</span>
          <span className="ml-2">{indicator.formula}</span>
        </div>
        <div className="rounded-md border border-[#c9b187]/40 bg-[#f4eccf]/56 px-3 py-2">
          <span className="font-black text-[#2e2418]">증빙</span>
          <span className="ml-2">{indicator.evidence}</span>
        </div>
        <div className="rounded-md border border-[#c9b187]/40 bg-[#f4eccf]/56 px-3 py-2">
          <span className="font-black text-[#2e2418]">참고</span>
...
export function DashboardMonitor({ activeMode: initialActiveMode, summary }: { activeMode: Mode; summary: DashboardSummary }) {
  ...
  return (
    <main
      className="dashboard-shell min-h-screen text-[#2e2418] lg:h-screen lg:overflow-hidden"
      data-live-summary-endpoint={dashboardSummaryEndpoint}
      data-live-summary-polling={dashboardSummaryPollingMs}
      data-visual-reference="votive-sanctuary-vintage"
    >
      ...
      <div className="flex min-h-screen flex-col px-3 py-3 sm:px-4 lg:h-screen">
        <header className="dashboard-header shrink-0 border-b border-[#c9b187]/70 pb-2">
          <div className="flex flex-col gap-2 xl:flex-row xl:items-end xl:justify-between">
            <div>
              <div className="dashboard-kicker text-[11px] font-black uppercase">KMUST · September 2018</div>
